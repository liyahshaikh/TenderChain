    ...
    registerTender: function() {
     ...
     var bidAmount = $("#bidAmount").val();
      ...
      App.contracts.TenderChain.deployed().then(function(instance) {
        ...
        return tenderchainInstance.createBid(tenderId, bidAmount, {from: App.account});
      }).then(function(result) {
          console.log("Done!")
        var bidProducedEvent = tenderchainInstance.bidProduced();
        bidProducedEvent.watch(function(error, result){
          if (!error) {
            $("#loadingOverlay").hide();
            showSuccessSnackbar();
            console.log("Created: "+result.args.bidId);
